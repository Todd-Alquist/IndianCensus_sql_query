...
from Dataset1 group by state order by avg_sex_ratio asc;
...
a.literacy/100 literacy_ratio,b.population 
from Dataset1 a 
...
from
(select d.district,
d.state,round(d.population/(1+d.growth),0) previous_census_population,
d.population current_census_population 
from
(select a.district,a.state,a.growth growth,
b.population from Dataset1 a 
...
from Dataset2)z) r on q.keyy=r.keyy)g

--window 

--output top 3 districts 
--from each state with highest literacy rate
